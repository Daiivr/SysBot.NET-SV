...
public class SudoModule<T> : ModuleBase<SocketCommandContext> where T : PKM, new()
{
    ...
    [RequireSudo]
    public async Task BlackListIDs([Summary("ID de discord separados por comas")][Remainder] string content)
    {
    ...
    [RequireSudo]
    public async Task UnBlackListIDs([Summary("ID de discord separados por comas")][Remainder] string content)
    {
        var IDs = GetIDs(content);
    ...
    [Summary("Imprime la lista de usuarios de Discord incluidos en la lista negra.")]
    [RequireSudo]
    ...
    [RequireSudo]
    public async Task BanOnlineIDs([Summary("ID en línea separados por comas")][Remainder] string content)
    {
        var IDs = GetIDs(content);
        var objects = IDs.Select(GetReference);
    ...
    [Summary("Prohíbe las identificaciones de usuarios en línea.")]
    [RequireSudo]
    public async Task UnBanOnlineIDs([Summary("ID en línea separados por comas")][Remainder] string content)
    {
        var IDs = GetIDs(content);
        var me = SysCord<T>.Runner;
        var hub = me.Hub;
    ...
    [Command("forgetUser")]
    [Alias("forget")]
    [Summary("Olvida a los usuarios que se encontraron anteriormente.")]
    [RequireSudo]
    public async Task ForgetPreviousUser([Summary("ID en línea separados por comas")][Remainder] string content)
    {
        var IDs = GetIDs(content);
