        ...
        TradeAbuseSettings AbuseSettings, CancellationToken token)
    {
        ...
        if (previous != null)
        {
            ...
            if (cd != 0 && TimeSpan.FromMinutes(cd) > delta)
            {
                ...
                poke.Notifier.SendNotification(bot, poke, $"⚠️ Sigues en tiempo de reutilización y no puedes intercambiar por otros **{wait.TotalMinutes:F1}** minuto(s).");
                var msg = $"⚠️ Encontre a **{user.TrainerName}{useridmsg}** ignorando el enfriamiento de tradeo de **{cd}** minutos. Encontrado por última vez hace **{delta.TotalMinutes:F1}** minutos.";
        ...
        if (!isDistribution)
        {
            ...
            if (previous_remote != null && previous_remote.Name != TrainerName)
            {
                ...
                var msg = $"⚠️ Encontre a **{user.TrainerName}{useridmsg}** __enviando__ a varios jugadores en el juego. **OT Anterior**: {previous_remote.Name}, **OT actual**: {TrainerName}";
